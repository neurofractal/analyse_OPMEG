...
% cfg = [];
% cfg.latency = [0 200];
% rawData_detrend = ft_selectdata(cfg,rawData_detrend);

% cfg = [];
% cfg.bsfilter    = 'yes';
% cfg.bsfreq      = [115 125];
% reference_channels         = ft_preprocessing(cfg,reference_channels);
% 
% cfg = [];
% cfg.bsfilter    = 'yes';
% cfg.bsfreq      = [97 103];
...
[u, u_hat, omega] = MVMD_new(data, 2000, 0, 6, 0, 1, 1e-7);

for i = 1:size(u,1)
    emd_spare = [];
    emd_spare = reference_channels;
    emd_spare.trial{1} = transpose(squeeze(u(i,:,:)));
    
    d = 1/reference_channels.fsample;
    emd_spare.time{1} = reference_channels.time{1}(1:size(u,2));  
    ...
    cfg.foi             = [0 150];
    cfg.plot            = 'yes';
    pow                 = ft_opm_psd(cfg,emd_spare);
    title(num2str(i));
    
    cfg= [];
    cfg.viewmode = 'vertical';
    cfg.blocksize = 10;
    ft_databrowser(cfg,emd_spare);
...
% 
% cfg = [];
% cfg.refchannel = emd_spare.label;
% cfg.channel    = meg_data.label;
% [dataout] = ft_denoise_pca(cfg, dataout,emd_spare);
% 
% end
% 
% cfg= [];
...
cfg.foi             = [0 150];
cfg.plot            = 'yes';
pow                 = ft_opm_psd_compare(cfg,data_out2,data_out);
...
cfg.channel = vertcat(ft_channelselection_opm('MEG',rawData),...
     ...
     '-N3-RAD','-MV-RAD');
cfg.refchannel = 'MEGREF';
cfg.filter_ref = [0 2 30; 40 60; 60 80];
cfg.derivative = 'yes';
cfg.return_all = 'no';
cfg.winsize    = 50;
...
cfg = [];
cfg.detrend = 'yes';
data = ft_redefinetrial(banana,data_out2);
