function grad = extractScannercastPosOri(folder,slotOri)
...
for sensorIdx = 1:size(sensorListing, 1)
    ...
    meanPos(sensorIdx,1:3)  = mean(verts,2);
end
headCentreVert  = mean(meanPos,2);
...
for sensorIdx = 1:size(sensorListing, 1)
    ...
    for redVertIdx = 1:length(redVerts(:,1))
        vertRepeatIdx           = ismember(verts,redVerts(redVertIdx,:),'rows');
        faceRepeatIdx           = ismember(faces,vertRepeatIdx);
    ...
    % Need to round it first to avoid small errors.
    meanVertDist        = round(meanVertDist,3);
    cornerVertsIdx      = (meanVertDist == max(meanVertDist));
    cornerVerts         = redVerts(cornerVertsIdx);
    ...
    for cornerIdx = 1:length(cornerVerts(:,1))
        cornerCentreDist(cornerIdx,1:3) = pdist([cornerVerts(cornerIdx,1:3);headCentreVert]);
    end
    
    % The closest four will be the bottom corners.
    bottomCornerIdx     = (cornerCentreDist < median(cornerCentreDist));
    bottomCornerVerts   = cornerVerts(bottomCornerIdx);
    topCornerVerts      = cornerVerts(~bottomCornerIdx);
    ...
    % Add orientation (tan)
    radOri(sensorIdx,1:3)       = coneVert - centrePoint(sensorIdx,1:3);
        
    % Add orientation (rad)
    tanOri(sensorIdx,1:3)       = coneVert - bottomEndPoint(1:3);
    ...
    sensorMesh.Faces{sensorIdx}(:,1:3)  = redFaces;
    sensorMesh.Points{sensorIdx}(:,1:3) = redVerts;
end
...
%% Normalise
for sensorIdx = 1:length(radOri(:,1))
    L               = sqrt(radOri(sensorIdx,1)^2 + radOri(sensorIdx,2)^2 + radOri(sensorIdx,3)^2);
    normZrad(sensorIdx,1:3) = radOri(sensorIdx,1:3)/L;
    L               = sqrt(tanOri(sensorIdx,1)^2 + tanOri(sensorIdx,2)^2 + tanOri(sensorIdx,3)^2);
    normZtan(sensorIdx,1:3) = tanOri(sensorIdx,1:3)/L;
end
